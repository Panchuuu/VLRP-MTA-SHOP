...
const statusColors = {
  completed: 'bg-green-950/50 text-green-400 border-green-800/50',
...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Usuarios', value: stats.users_total, color: 'text-purple-400' },
    { label: 'Productos activos', value: stats.products_active, color: 'text-slate-300' },
  ];
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        {cards.map((c) => (
          <div key={c.label} className="bg-[#0f0f1a] border border-[#1e1e30] rounded-xl p-5">
      ...
      <h2 className="text-lg font-semibold text-white mb-4">Últimas órdenes</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {stats.recent_orders.map((o) => (
              <tr key={o.id} className="border-b border-[#1e1e30]/50 hover:bg-[#13132a]">
                <td className="px-4 py-3 text-slate-300">{o.username}</td>
