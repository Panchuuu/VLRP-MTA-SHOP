...
export default function AdminTestimonials() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Testimonios</h1>
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-[#0f0f1a] border border-slate-200 dark:border-[#1e1e30] rounded-xl h-28 animate-pulse" />
          ))}
        </div>
      ) : items.length === 0 ? (
        ...
        <div className="space-y-4">
          {items.map((t) => (
            <div
              key={t.id}
              className="bg-white dark:bg-[#0f0f1a] border border-slate-200 dark:border-[#1e1e30] rounded-xl p-5 flex flex-col sm:flex-row sm:items-center gap-4"
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  />
                  <span className="text-slate-700 dark:text-slate-300 text-sm font-medium">{t.author_name}</span>
                  <span className="text-yellow-400 text-xs">
                    {'★'.repeat(t.rating)}
                    <span className="text-slate-700">{'★'.repeat(5 - t.rating)}</span>
                  </span>
                </div>
                <p className="text-slate-600 dark:text-slate-400 text-sm">"{t.content}"</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-lg font-medium ${
                    t.is_approved
                      ? 'bg-slate-50 dark:bg-[#080810] border border-slate-200 dark:border-[#1e1e30] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
